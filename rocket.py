        ...
        for p in self.n_body_system.p: #for all particles
            ...
            for q in self.n_body_system.p: #sum
                ...
                if (q.i != n):
                    u = self.U(q.r,p.r)
                    r = self.norm(u)
                    if (r <= q.radii +  p.radii):#En esta linea hay que quitar el = del <=
...
dt = 2e-2 #sec
#prueba1 dt= 1e-2
earth = Particle(5.9736e24, 6371.0e3 ,  [0.0, 0.0, -6371.0e3], [0.0, 0.0, 0.0])
#ball = Particle(0.450, 0.025, [84.58, 32.91, 1.85], [7.69, 7.7572, 2])
ball = Particle(0.450, 0.025, [84.0, 32.0, 2.0], [7.69, 7.75, 0])
ball_r = Particle(0.450, 0.025, [84.5, 32.5, 2.0], [10.45, 10.91, 0])
#prueba1 r[]
parabolic_system =  N_body_system()

parabolic_system.add_particle(earth)
parabolic_system.add_particle(ball)
parabolic_system.add_particle(ball_r)
newton = Integrator(parabolic_system, dt)

parabolic_system.Header()
skip = 0

for j in range(1000): #300*.7 = 210 sec > 300 sec
